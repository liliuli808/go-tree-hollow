...
func Routes(r *gin.RouterGroup, handler *Handler, likeHandler *LikeHandler, commentHandler *CommentHandler, authMiddleware gin.HandlerFunc) {
	// 公开路由组（不需要认证）
	publicPosts := r.Group("/posts")
	{
		publicPosts.GET("/:id", handler.GetPost)                      // GET /api/v1/posts/:id - 获取单个帖子
		publicPosts.GET("/:id/like/status", likeHandler.GetLikeStatus) // GET /api/v1/posts/:id/like/status - 获取点赞状态
